...
void main()
{
	...
	hdotn[0] = max(dot(modelview * vec4(normal,0), normalize(normalize(lightPosition[0]-(modelview * position)) - normalize(modelview * position))),0);
	hdotn[1] = max(dot(modelview * vec4(normal,0), normalize(normalize(lightPosition[1]-(modelview * position)) - normalize(modelview * position))),0);
