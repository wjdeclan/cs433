...
public class GLRenderContext implements RenderContext {
	...
	/**
	 * Set the scene manager. The scene manager contains the 3D scene that will be
	 * rendered. The scene includes geometry as well as the camera and viewing
	 * frustum.
	 */
	...
	/**
	 * This method is called by the GLRenderPanel to redraw the 3D scene. The method
	 * traverses the scene using the scene manager and passes each object to the
	 * rendering method.
	 */
	...
	/**
	 * This method is called at the end of each frame, i.e., after scene drawing is
	 * complete.
	 */
	...
	private void draw(RenderItem renderItem) {
		...
		gl.glDrawElements(GL3.GL_TRIANGLES, renderItem.getShape().getVertexData().getIndices().length,
	...
	private void initArrayBuffer(GLVertexData data) {
		...
		while (itr.hasNext()) {
			...
			// Upload vertex data
			gl.glBufferData(GL3.GL_ARRAY_BUFFER, e.getData().length * 4, FloatBuffer.wrap(e.getData()),
					GL3.GL_DYNAMIC_DRAW);
		...
		gl.glBufferData(GL3.GL_ELEMENT_ARRAY_BUFFER, data.getIndices().length * 4, IntBuffer.wrap(data.getIndices()),
				GL3.GL_DYNAMIC_DRAW);
	...
	private void setTransformation(Matrix4f transformation) {
		...
		gl.glUniformMatrix4fv(gl.glGetUniformLocation(activeShaderID, "modelview"), 1, false,
	...
	/**
	 * Set up a material for rendering. Activate its shader, and pass the material
	 * properties, textures, and light sources to the shader.
	 * 
	 * @param m
	...
	private void setMaterial(Material m) {
		...
		if (m != null && m.shader != null) {
			...
			String lightString = "lightPosition[" + 0 + "]";
			id = gl.glGetUniformLocation(activeShaderID, lightString);
			if (id != -1)
				gl.glUniform4f(id, 0, 0, 1, 0.f); // Set light position
			//else
				//System.out.print("Could not get location of uniform variable " + lightString + "\n");
			int nLights = 1;

			// Iterate over all light sources in scene manager (overwriting the default
			// light source)
			...
			if (iter != null) {
				...
				while (iter.hasNext() && nLights < 8) {
					...
					// Pass light position to shader, we assume the shader stores it in an array
					// "lightPosition[]"
					...
					// Pass light color to shader, we assume the shader stores it in an array
					// "lightColor[]"
					lightString = "lightColor[" + nLights + "]";
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if (id != -1) {
						gl.glUniform3f(id, l.diffuse.x, l.diffuse.y, l.diffuse.z); // Set light direction
					}
					//else
						//System.out.print("Could not get location of uniform variable " + lightString + "\n");
				...
				// Pass number of lights to shader, we assume this is in a variable "nLights" in
				// the shader
	...
	/**
	 * Convert a Transformation to a float array in column major ordering, as used
	 * by OpenGL.
	 */
